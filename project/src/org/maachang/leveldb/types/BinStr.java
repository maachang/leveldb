 ...
 * バイナリ、文字列の２キー情報.
 */
public final class BinStr extends TwoKey {
	 ...
	 *            2つ目のキーを設定します.
	 */
	public BinStr(Object one, Object two) {
		this.set(one, two);
	}
	 ...
	 *                例外.
	 */
	public BinStr(JniBuffer buf) throws Exception {
		create(buf);
	}
	 ...
	 * @exception Exception.
	 */
	public BinStr(JniBuffer buf, int off, int len) throws Exception {
		create(buf, off, len);
	}
	...
	public final int toBufferLength() {
		return one.length + NativeString.nativeLength(two) + 4;
	}
	...
	public final int compareTo(Object o) {
		...
		if (o instanceof BinStr) {
			...
			throw new LeveldbException("比較対象のオブジェクトは、1キーがバイナリではありません");
		} else if (o instanceof byte[]) {
	...
	public final String toString() {
		return new StringBuilder("[bin-str]")
