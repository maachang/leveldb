...
/**
 * Number32(int)、文字列の２キー情報.
 */
public final class IntStr extends TwoKey {
	 ...
	 *            2つ目のキーを設定します.
	 */
	public IntStr(Object one, Object two) {
		this.set(one, two);
	}
	 ...
	 *                例外.
	 */
	public IntStr(JniBuffer buf) throws Exception {
		create(buf);
	}
	 ...
	 * @exception Exception.
	 */
	public IntStr(JniBuffer buf, int off, int len) throws Exception {
		create(buf, off, len);
	}
	...
	public final void create(JniBuffer buf, int off, int len) throws Exception {
		...
		if (len <= 4) {
			...
			two = NativeString.toJava(addr, 4, len - 4);
		}
	}
	...
	public final int toBufferLength() {
		return 4 + NativeString.nativeLength(two);
	}
	...
	public static final void convertBuffer(Object one, Object two, JniBuffer buf) throws Exception {
		...
		// two.
		if (len2 != 0) {
			NativeString.toNative(addr, pos + 4, (String) two);
		}
