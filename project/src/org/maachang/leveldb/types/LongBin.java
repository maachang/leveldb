...
/**
 * Number64(long)、バイナリの２キー情報.
 */
public final class LongBin extends TwoKey {
	 ...
	 *            2つ目のキーを設定します.
	 */
	public LongBin(Object one, Object two) {
		this.set(one, two);
	}
	 ...
	 *                例外.
	 */
	public LongBin(JniBuffer buf) throws Exception {
		create(buf);
	}
	 ...
	 * @exception Exception.
	 */
	public LongBin(JniBuffer buf, int off, int len) throws Exception {
		create(buf, off, len);
	}
	...
	public final void create(JniBuffer buf, int off, int len) throws Exception {
		...
		if (len == 0) {
			...
			two = new byte[len];
			JniIO.getBinary(addr, 8, two, 0, len);
		}
	}
