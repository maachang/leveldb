 ...
 */
public final class IntBin extends TwoKey {
	 ...
	 *            2つ目のキーを設定します.
	 */
	public IntBin(Object one, Object two) {
		this.set(one, two);
	}
	 ...
	 *                例外.
	 */
	public IntBin(JniBuffer buf) throws Exception {
		create(buf);
	}
	 ...
	 * @exception Exception.
	 */
	public IntBin(JniBuffer buf, int off, int len) throws Exception {
		create(buf, off, len);
	}
	...
	public final void create(JniBuffer buf, int off, int len) throws Exception {
		...
		if (len == 0) {
			...
			two = new byte[len];
			JniIO.getBinary(addr, 4, two, 0, len);
		}
	}
	...
	public final String toString() {
		return new StringBuilder("[num32-bin]")
				.append(one)
				.append(" : ")
